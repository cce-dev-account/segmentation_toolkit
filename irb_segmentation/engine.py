...
class IRBSegmentationEngine:
    ...
    def fit(
        ...
        # Validate training arrays
        validate_array(
            X,
            name="X_train",
            min_samples=self.params.min_samples_leaf * 3,
            min_features=1,
            allow_nan=False,
            allow_inf=False
        )
        ...
        n_samples, n_defaults, default_rate = validate_binary_target(
            ...
            name="y_train",
            min_samples=self.params.min_samples_leaf * 3,
            min_defaults=self.params.min_defaults_per_leaf * 3,
            max_default_rate=0.50
        )

        # Validate validation set if provided
        if X_val is not None and y_val is not None:
            validate_array(
                X_val,
                name="X_val",
                min_samples=50,  # Less strict for validation set
                min_features=X.shape[1],  # Must match training features
                allow_nan=False,
                allow_inf=False
            )
            ...
            validate_binary_target(
                ...
                name="y_val",
                min_samples=50,
                min_defaults=5,  # Less strict for validation set
                max_default_rate=0.50
            )

            # Check train/val compatibility
            validate_train_val_compatibility(X, y, X_val, y_val)

        elif (X_val is None) != (y_val is None):
            # One is provided but not the other
            raise ValueError(
                "Both X_val and y_val must be provided together, or both must be None. "
                f"Got X_val={'provided' if X_val is not None else 'None'}, "
                f"y_val={'provided' if y_val is not None else 'None'}"
            )

        # Validate and standardize feature names
        ...
        self.X_val_ = X_val
        self.y_val_ = y_val
        self.feature_names_ = validated_feature_names
        self.X_categorical_ = X_categorical

        logger.info(f"Training segmentation model on {len(X)} observations")
        logger.info(f"  Default rate: {np.mean(y):.4f}")
        ...
        logger.info(f"Initial tree created {len(np.unique(self.segments_train_))} segments")
    ...
    def _apply_business_constraints(
        ...
        logger.info("Applying forced splits")
        ...
        for feature_name, threshold in self.params.forced_splits.items():
            if isinstance(feature_name, str):
                try:
                    ...
                    logger.warning(f"Feature '{feature_name}' not found, skipping forced split")
                    continue
            else:
        ...
        if forced_splits_indexed:
            ...
            logger.info(f"  Applied {len(split_log)} forced splits")
    ...
    def _enforce_irb_constraints(
        ...
        Returns:
            Adjusted segments
        """
        logger.info("Enforcing IRB constraints")
        ...
        for iteration in range(max_iterations):
            ...
            if needs_merge:
                ...
                logger.info(f"    Merged {len(merge_log)} segments")
            ...
            if not density_issues.get('passed', True):
                ...
                if large_segments:
                    logger.info("  Splitting large segments")
                    ...
                    logger.info(f"    Split {len(split_log)} segments")
        ...
        if self.params.monotone_constraints:
            logger.info("Checking monotonicity constraints")
            ...
            if violation_log:
                logger.warning(f"  Found {len(violation_log)} monotonicity violations")
            else:
                logger.info("  [PASS] All monotonicity constraints satisfied")
    ...
    def predict(self, X: np.ndarray, X_categorical: Optional[Dict[str, np.ndarray]] = None) -> np.ndarray:
        ...
        validate_array(
            ...
            name="X",
            min_samples=1,
            min_features=self.X_train_.shape[1],  # Must match training features
            allow_nan=False,
            allow_inf=False
        )
    ...
    def _print_fit_summary(self):
        ...
        logger.info(f"Final segments: {len(np.unique(self.segments_train_))}")

        # Print segment statistics
        stats = self._get_segment_statistics()
        logger.info("Segment Statistics:")
        logger.info(f"{'Segment':<10}{'Count':<10}{'Defaults':<12}{'PD Rate':<12}{'Density':<10}")
        logger.info("-" * 60)
        ...
        if val_train.get('all_passed'):
            logger.info("  [PASS] All training validations passed")
        else:
            logger.warning("  [FAIL] Some training validations failed:")
        ...
        if 'validation' in self.validation_results_:
            ...
            if val_val.get('all_passed'):
                logger.info("  [PASS] All validation set checks passed")
            else:
                logger.warning("  [FAIL] Some validation set checks failed")

        # Print adjustment summary
        logger.info("Adjustments Applied:")
        logger.info(f"  Merges: {len(self.adjustment_log_['merges'])}")
        logger.info(f"  Splits: {len(self.adjustment_log_['splits'])}")
        logger.info(f"  Forced splits: {len(self.adjustment_log_['forced_splits'])}")
        logger.info(f"  Monotonicity violations: {len(self.adjustment_log_['monotonicity_violations'])}")
    ...
    def get_comprehensive_segment_descriptions(self) -> Dict[int, Dict]:
        """
        Generate comprehensive descriptions for all segments including those created
        through post-processing adjustments.

        Returns:
            Dictionary mapping segment ID to description dict containing:
            - statistics: observations, defaults, default_rate, density
            - tree_rule: IF-THEN rule from tree (if available)
            - feature_profile: Statistical profile of key features
            - adjustment_history: How this segment was created/modified
        """
        if not self.is_fitted_:
            raise RuntimeError("Model must be fitted before extracting descriptions")
        ...
        for leaf, segment in zip(train_leaves, self.segments_train_):
            leaf = int(leaf)
            segment = int(segment)
            if segment not in leaf_to_segment:
                leaf_to_segment[segment] = []
            if leaf not in leaf_to_segment[segment]:
                leaf_to_segment[segment].append(leaf)

        # Extract tree path rules for each leaf
        leaf_rules = {}
        ...
        def extract_path(node, path=""):
            if tree.feature[node] != _tree.TREE_UNDEFINED:
                ...
                # Left child (<=)
                left_path = f"{path} AND {feature_name} <= {threshold:.4f}" if path else f"{feature_name} <= {threshold:.4f}"
                extract_path(tree.children_left[node], left_path)

                # Right child (>)
                right_path = f"{path} AND {feature_name} > {threshold:.4f}" if path else f"{feature_name} > {threshold:.4f}"
                extract_path(tree.children_right[node], right_path)
            else:
                # Leaf node
                leaf_rules[int(node)] = path

        extract_path(0)

        # Build comprehensive descriptions for each segment
        unique_segments = np.unique(self.segments_train_)
        descriptions = {}
        ...
        for seg in unique_segments:
            ...
            n_defaults = int(np.sum(self.y_train_[mask]))
            default_rate = float(np.mean(self.y_train_[mask]))
            density = float(n_obs / len(self.segments_train_))

            # Try to get tree rule
            tree_rule = None
            if seg in leaf_to_segment:
                leaves = leaf_to_segment[seg]
                if len(leaves) == 1:
                    # Simple case: segment maps to single leaf
                    tree_rule = leaf_rules.get(leaves[0], None)
                elif len(leaves) > 1:
                    # Multiple leaves merged into this segment
                    # Store ALL individual rules for audit purposes
                    rules = [leaf_rules.get(leaf, None) for leaf in leaves if leaf_rules.get(leaf)]
                    if rules:
                        tree_rule = {
                            'type': 'multiple',
                            'n_paths': len(rules),
                            'paths': rules,
                            'leaf_nodes': leaves  # Store leaf node IDs for reference
                        }

            # Feature profile - calculate percentiles and medians for all features
            X_seg = self.X_train_[mask]
            feature_profile = {}

            for feat_idx, feat_name in enumerate(self.feature_names_):
                feat_values = X_seg[:, feat_idx]
                feature_profile[feat_name] = {
                    'median': float(np.median(feat_values)),
                    'p5': float(np.percentile(feat_values, 5)),
                    'p95': float(np.percentile(feat_values, 95)),
                    'mean': float(np.mean(feat_values)),
                    'std': float(np.std(feat_values))
                }

            # Identify top discriminative features for this segment
            # Compare to overall population medians
            discriminative_features = []
            for feat_idx, feat_name in enumerate(self.feature_names_):
                seg_median = feature_profile[feat_name]['median']
                global_median = float(np.median(self.X_train_[:, feat_idx]))

                # Calculate relative difference
                if abs(global_median) > 1e-10:
                    rel_diff = abs(seg_median - global_median) / abs(global_median)
                else:
                    rel_diff = abs(seg_median - global_median)

                discriminative_features.append((feat_name, rel_diff, seg_median, global_median))

            # Sort by discriminative power and take top 5
            discriminative_features.sort(key=lambda x: x[1], reverse=True)
            top_features = discriminative_features[:5]

            # Get adjustment history for this segment
            adjustment_history = self._get_segment_adjustment_history(seg)
    ...
    def _get_segment_adjustment_history(self, segment_id: int) -> Dict:
        """
        Extract adjustment history for a specific segment.

        Args:
            segment_id: Segment to get history for
        ...
        history = {
            ...
            'involved_in_merges': [],
            'has_monotonicity_violations': []
        }

        # Check merges
        for merge in self.adjustment_log_['merges']:
            if merge['into_segment'] == segment_id:
                history['created_by_merges'].append({
                    'merged_segment': merge['merged_segment'],
                    'reason': merge['reason'],
                    'original_defaults': merge.get('original_defaults', 'unknown')
                })
            elif merge['merged_segment'] == segment_id:
                history['involved_in_merges'].append({
                    'merged_into': merge['into_segment'],
                    'reason': merge['reason']
                })

        # Check splits
        for split in self.adjustment_log_['splits']:
            if split.get('original_segment') == segment_id:
                history['created_by_splits'].append({
                    'new_segments': split.get('new_segments', []),
                    'reason': split.get('reason', 'unknown')
                })

        # Check forced splits
        for forced_split in self.adjustment_log_['forced_splits']:
            if forced_split.get('new_segment') == segment_id or forced_split.get('segment') == segment_id:
                history['created_by_forced_splits'].append({
                    'feature': forced_split.get('feature_name', 'unknown'),
                    'threshold': forced_split.get('threshold', 'unknown'),
                    'parent_segment': forced_split.get('segment', 'unknown')
                })

        # Check monotonicity violations
        for violation in self.adjustment_log_['monotonicity_violations']:
            if violation.get('segment1') == segment_id or violation.get('segment2') == segment_id:
                history['has_monotonicity_violations'].append({
                    'feature': violation.get('feature_name', 'unknown'),
                    'direction': violation.get('direction', 'unknown'),
                    'other_segment': violation.get('segment2' if violation.get('segment1') == segment_id else 'segment1', 'unknown')
                })
    ...
    def export_tree_to_file(self, filepath: str):
        ...
        logger.info(f"Tree structure exported to: {filepath}")
